<script>
  ...
  function draw() {
    ...
    const bitmapPattern = new createjs.Bitmap(patternImage);
    ...
    */
    ...
    cadre.graphics.beginFill('#333').rect(0,0, width, 10);
    cadre.y = height - 10;
    ...
    for (let i = 1; i < total; i++) {
      ...
      tick.y = y
      stage.addChild(tick)
      const digit = new createjs.Text(i+'h', '12px helvetica', '#FFF')
      digit.x = x
      digit.y = cadre.y + 2;
      digit.scaleY = 0.6
